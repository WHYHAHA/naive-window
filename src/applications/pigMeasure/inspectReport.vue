        ...
        <n-card>
            <n-grid x-gap="12" cols="10" item-responsive>
                    ...
                    <div class="tabHeader font-16 font-weight-bolder">
                        <n-space justify="space-between">
                            <n-space align="center">
                                ...
                                <n-button type="info" @click="createInpectModal = true">{{ $t('dtables.listing.createInspectionSubmissionForm') }}</n-button>
                            </n-space>
                        </n-space>
                    </div>
                    <!-- <n-data-table
                ...
                <n-gi span="4 600:4 600:4 800:6">
                    <inspectPending
                        @getData="params => { getData(params),search.page = 1}"
                        @refershList="params => {search.page = 1,getPigInspectList(params)}"
                        :innerHeight="innerHeight-10"
                        :originData="inspectDetailData"
                        v-if="inspectDetailData.type === 'inspectPending'"
                    />
                    <inspectComplete
                        @getData="getPigInspectList(),search.page = 1, inspectDetailData = {type: false,id: '',inspectionCode: ''}"
                        @refershList="params => {search.page = 1,getPigInspectList(params)}"
                        :innerHeight="innerHeight-10"
                        :originData="inspectDetailData"
                    ...
                    <n-empty class="mt-3 empty-table" v-if="!inspectDetailData.type" :description="$t('dtables.listing.noContent')"></n-empty>
                </n-gi>
            </n-grid>
        </n-card>
        ...
        >
            <n-form :model="createInspectForm" ref="createInspectRef" label-placement="left" :label-width="100">
                <n-form-item path="pigCodeMain" :label="$t('dtables.listing.plannedInspectionDate')" :require="true">
                    <n-date-picker v-model:value="createInspectForm.planInspectionDatetime" type="date" :is-date-disabled="disablePreviousDate" />
                </n-form-item>
                <n-form-item path="determinationType" :label="$t('dtables.listing.theDeterminationOfTheType')" :require="true">
                    <n-select filterable v-model:value="createInspectForm.determinationType" :options="measureOptions" :style="{width:'180px'}" />
                </n-form-item>
                <n-space justify="center">
                    <n-button type="info" @click="handleCreateInspect">{{ $t('dataTable.listing.confirm') }}</n-button>
                    <n-button @click="createInpectModal=false; ">{{ $t('dtables.listing.cancel') }}</n-button>
                </n-space>
            </n-form>
        </n-modal>
        <!-- 修改 -->
        <n-modal class="custom-card" v-model:show="showUpdate" preset="card" :style="bodyStyle" :title="$t('dataTable.listing.optionalBoar')" :bordered="false" @afterLeave="clearForm()">
            <n-form :model="updateForm" ref="formRef" :rules="rules" label-placement="left" :label-width="100">
                <n-form-item path="pigCodeMain" :label="$t('dataTable.listing.optionalBoarMaster')">
                    <n-input-group>
                        <n-input :style="{ width: '300px' }" maxlength="20" :placeholder="$t('dataTable.listing.pleaseEnterTheBoar (main)Encoding')" v-model:value="updateForm.pigCodeMain" />
                        <n-button type="primary" @click="getMatchMessage('main')">{{$t('dtables.listing.search')}}</n-button>
                    </n-input-group>
                </n-form-item>
                <n-space vertical v-if="mainPig.breedingValue" style="padding-left:100px;margin-bottom:30px">
                    <n-text depth="3">{{$t('dataTable.listing.breedingValue') + mainPig.breedingValue}}</n-text>
                    <n-text depth="3">{{ $t('dataTable.listing.semenCode') + mainPig.repertoryAmount}}</n-text>
                </n-space>
                <n-form-item path="pigCodePrepare" :label="$t('dataTable.listing.boar (prepare)')">
                    <n-input-group>
                        <n-input :style="{ width: '300px' }" maxlength="20" :placeholder="$t('dataTable.listing.pleaseEnterTheBoar (prepare)Encoding')" v-model:value="updateForm.pigCodePrepare" />
                        <n-button type="primary" @click="getMatchMessage('prepare')">{{$t('dtables.listing.search')}}</n-button>
                    </n-input-group>
                </n-form-item>
                <n-space vertical v-if="preparePig.breedingValue" style="padding-left:100px;margin-bottom:30px">
                    <n-text depth="3">{{$t('dataTable.listing.breedingValue') +preparePig.breedingValue}}</n-text>
                    <n-text depth="3">{{ $t('dataTable.listing.semenCode') + preparePig.repertoryAmount}}</n-text>
                </n-space>
                <n-space justify="center">
                    <n-button type="info" @click="handleUpdateClick">{{ $t('dataTable.listing.confirm') }}</n-button>
                    <n-button @click="showUpdate=false; ">{{ $t('dtables.listing.cancel') }}</n-button>
                </n-space>
            </n-form>
        </n-modal>
    </div>
            ...
            let methods = {
                ...
                handleCreateInspect() {
                    ...
                    if (!state.createInspectForm.determinationType) {
                        message.error(t("dataTable.listing.testTypeCannotBeEmpty"))
                        return false
                    }
                    ...
                    formRef.value.validate(errors => {
                        if (!errors) {
                            ...
                            if (state.updateForm.pigCodePrepare) {
                                methods.getMatchMessagelists("prepare", state.updateForm.pigCodePrepare)
                            }
                        }
                    })
                },
