<template lang="">
    <div class='position-relative' >
         <div class="appFolder" v-for="(item, index) in deskBox" :key="index" :style="{
                height: item.height + '%',
                width: item.width + '%',
                top: item.top + 'px',
                left: 0,
                ...
                <n-space justify="space-between" align="center" class="appFolderheard">
                    <div class="flex align-center">
                        <div class="font-14 ml-1">{{item.boxName}}</div>
                    </div>
                    <div class="addIcon animated" v-if="item.popoverFlag">
                        <n-popover :overlap="overlap" :style="{ width: '300px',padding:0 }" placement="bottom-start" trigger="click" :show-arrow="false">
                            <template #trigger>
                ...
                <div class="appFolderBody">
                    <!-- BI连接数据图 -->
                     <!-- <n-grid x-gap="12" y-gap="8" cols="3">
                        <n-gi v-for="(applistItem,index) in item.appList" :key="index">
                            <webView v-if="applistItem.type == 'webView'" :setting='applistItem'></webView>
                        </n-gi>
                    </n-grid> -->
                    <div v-for="(applistItem,index) in item.appList" :key="index" class='w-100'>
                        <WebView v-if="applistItem.type == 'webView'" :setting='applistItem'></WebView>
                    </div>
                       
                </div>
                <div class="resize-overlay" v-show="overlayShow"></div>
                <div class="resize-side" v-for="(value, index) in resizeSide" :key="index" v-show="item.useDrag" :class="value" @mousedown.self="onResize(value,item)"></div>
        </div>
...
<script>
    ...
    // import { AddSquare24Regular as JiahaoIcon } from "@vicons/fluent"
