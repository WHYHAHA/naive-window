    ...
    <n-card content-style="padding: 0;">
        <div class="tabHeader font-16 font-weight-bolder">
            <n-space justify="space-between">
                ...
                <n-space align="center">
                    <n-button type="success" @click="handleConfirmInspect">{{ $t('dtables.listing.completionOfInspection') }}</n-button>
                </n-space>
            </n-space>
        </div>
            ...
            let methods = {
                ...
                confirmAdd() {
                    let list = state.addSampleTableList
                        ...
                        .map(({ pigCode, subSampleCode, samplcollectDate }) => {
                            return {
                                pigCode,
                ...
                handleMutipleDel: () => {
                    ...
                    let list = state.dataList
                        ...
                        .map(t => ({ id: t.id, subSampleCode: t.subSampleCode }))
                                    ...
                                    h(
                                        NButton,
                                        {
                                            ...
                                            onClick: () => {
                                                deletePmInspectionDetail({
                                                    id: row.id,
                                                    subSampleCode: row.subSampleCode
                                                }).then(res => {
                                                    if (res.code == 0) {
                                                        emit("refershList")
                                                        methods.setTables()
