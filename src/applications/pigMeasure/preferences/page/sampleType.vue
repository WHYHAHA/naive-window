<template>
    <n-space vertical :size="0">
        ...
        <div class="p-2">
            <n-card title="偏好设置：采样类型设置">
                <n-grid :cols="2" :x-gap="12">
                    <n-gi>
                        <div class="tabHeader">
                            <n-space align="center" :size="20">
                                <div class="font-16 font-weight-bolder">采样类型</div>
                                <n-button type="info" class="col-2" @click="openAdd"
                                    >新建采样类型</n-button
                                >
                            </n-space>
                        </div>
                        <n-space vertical>
                            ...
                            <n-space justify="space-between" align="center">
                                <n-space justify="space-between">
                                    <n-text class="pl-1"
                                        >本页共：{{ measureTypeList.length }}条</n-text
                                    >
                                </n-space>
                                <n-pagination
                    ...
                    <n-gi>
                        <div class="tabHeader">
                            <n-space align="center" :size="0" justify="space-between">
                                <n-space align="center" :size="0">
                                    <div class="font-16 font-weight-bolder">采样类型详情</div>
                                </n-space>
                            </n-space>
                        </div>
                        ...
                        >
                            <n-space vertical :size="15">
                                ...
                                <n-card
                                    title="采样范围设置："
                                    v-if="measureTemplate['采样范围设置']"
                                >
                                    <n-form-item
                                        label="性别："
                                        v-if="measureTemplate['采样范围设置']['性别']"
                                    >
                                        <n-checkbox-group
                                    ...
                                    <n-form-item
                                        label="日龄："
                                        v-if="measureTemplate['采样范围设置']['日龄']"
                                    ...
                                    <n-form-item
                                        label="品种："
                                        v-if="measureTemplate['采样范围设置']['品种']"
                                    >
                                        <n-checkbox-group
                                ...
                                <n-card
                                    title="常规采样项目设置："
                                    v-if="measureTemplate['常规采样项目设置']"
                                >
                                    <n-form-item
                                        path="measuringProject.bloodPentathlon"
                                        v-if="measureTemplate['常规采样项目设置']['血五项']"
                                    >
                                        <n-checkbox-group
                                            v-model:value="form.measuringProject.bloodPentathlon"
                                        >
                                            <n-space item-style="display: flex;">
                                                <n-checkbox
                                                    v-for="list in getDictionaries(
                                                        'bloodPentathlon'
                                                    )"
                                                    :key="list.value"
                                                    :value="list.value"
    ...
    export default defineComponent({
        ...
        setup(props, { emit }) {
            ...
            const getMeasureTemplate = async () => {
                // id 1 获取采样模板
                await getMeasureTemplateApi({ id: 2 }).then(res => {
                    if (res.code == 0) {
